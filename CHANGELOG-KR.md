...
## [v0.5.39] - 2026-08-22
### 주요 변경 및 개선 사항 (현재 선택된 마커에 퍼져나가는 붉은 원 펄스 애니메이션 추가)
- **선택된 마커 붉은 원 펄스 애니메이션 적용 (`MapViewer.tsx`)**:
  - 현재 선택된 모임의 마커 핀(`isSelected === true`) 중심에서 바깥쪽으로 부드럽게 퍼져나가는 붉은색 원형 파동(`animate-ping` & `animate-pulse`) 효과를 적용하여 현재 위치가 지도 상에서 뚜렷하게 강조되도록 개선.

## [v0.5.38] - 2026-08-22
### 주요 변경 및 개선 사항 (사이드바 메뉴 모임 선택 시 2단계 상호작용 적용)
- **사이드바 모임 카드 클릭 상호작용 개선 (`Sidebar.tsx`)**:
  - 모임 목록에서 다른 모임을 최초 클릭했을 때: 상세 모달을 띄우지 않고 **해당 모임 선택 및 지도 좌표 이동(포커스)만 진행**.
  - **이미 선택되어 있는 모임을 다시 클릭**하거나 우측 하단의 **[상세보기]**를 누른 경우에만 **모임 상세 정보 모달이 출력**되도록 개선.

## [v0.5.37] - 2026-08-22
### 주요 변경 및 개선 사항 (후보 선택 팝업 활성화 시 모든 제목 레이블 자동 숨김)
- **후보 선택 팝업 중 모든 모임 제목 레이블 숨김 처리 (`MapViewer.tsx`)**:
  - 다중 모임 마커를 클릭하여 후보 선택 팝업(`activeGroupPopupKey !== null`)이 열려 있는 동안에는 지도 상에 떠 있던 이전 모임의 제목 레이블 및 모든 툴팁을 완전히 숨겨 팝업에만 시선이 집중되도록 개선.
  - 팝업에서 특정 모임을 선택하거나 팝업을 닫으면 다시 해당 선택 모임의 제목 레이블이 표시됩니다.

## [v0.5.36] - 2026-08-22
### 주요 변경 및 개선 사항 (겹쳐진 마커 배지 표시 제거)
- **마커 핀 배지 제거 (`MapViewer.tsx`)**:
  - 겹쳐진 마커 핀 우상단의 숫자 뱃지(`+N`) 출력을 제거하여 지도 마커 본연의 깔끔하고 미니멀한 원형 점 스타일 유지.

## [v0.5.35] - 2026-08-22
### 주요 변경 및 개선 사항 (다중 모임 마커 클릭 시 모임 출력 보류 및 팝업 선택 우선화)
- **다중 모임 마커 클릭 시 모임 자동 선택/출력 보류 (`MapViewer.tsx`)**:
  - 겹쳐진 마커(`+N`)를 클릭했을 때 최신 모임이 먼저 선택되어 모달/화면이 바뀌던 문제를 수정.
  - 마커 클릭 시 모임 출력을 **일단 보류**하고 오직 **후보 모임 선택 팝업만 오픈**되도록 개선.
  - 사용자가 팝업 안에서 원하는 모임을 클릭했을 때 비로소 해당 모임이 선택 및 출력됩니다.

## [v0.5.34] - 2026-08-22
### 주요 변경 및 개선 사항 (모임 선택/전환 시 사진 위젯 강제 ON 버그 해결)
- **사진 위젯 ON/OFF 사용자 설정 영구 유지 (`App.tsx`)**:
  - `handleSelectGathering`에서 모임 전환 시 사진 위젯 닫힘 상태를 강제로 해제하던 로직 제거.
  - 사용자가 사진 위젯을 닫아두었거나 TopBar에서 OFF로 설정한 경우, 다른 모임으로 전환되거나 자동 순환되더라도 **꺼진 상태가 그대로 유지**되도록 개선.

## [v0.5.33] - 2026-08-22
### 주요 변경 및 개선 사항 (동일 위치 다중 모임 겹침 시 후보 모임 선택 팝업 추가)
- **동일 위치 다중 모임 후보 선택 팝업 구현 (`MapViewer.tsx`)**:
  - 마커 핀 위치에 2개 이상의 모임이 겹쳐 있는 경우, 마커 우상단에 다중 모임 뱃지(`+N`)를 표시.
  - 해당 마커 클릭 시 해당 장소의 후보 모임 목록 팝업(`activeGroupPopupKey`)이 플로팅되어 원하는 회차의 모임을 직관적으로 선택할 수 있도록 개선.
  - 팝업 내에서 회차 뱃지, 모임 제목, 모임 일시가 시간 역순(최신순)으로 정렬되어 노출.

## [v0.5.32] - 2026-08-22
### 주요 변경 및 개선 사항 (사이드바 메뉴 오픈 시 모든 백그라운드 애니메이션 일시 정지)
- **메뉴 및 모달 열림 상태 애니메이션 일시 정지 연동 (`App.tsx`)**:
  - `isSidebarOpen` (사이드바 메뉴 오픈) 상태일 때 다음 백그라운드 애니메이션들을 일시 정지:
    1. **달력 위젯 자동 회차 순환 타이머** 일시 정지
    2. **좌하단 포토 위젯 3.5초 다중 사진 슬라이드 전환** 일시 정지
    3. **좌하단 포토 위젯 3D 물결 유영 및 파도 물리 타이머** 일시 정지
  - 메뉴를 닫으면 애니메이션이 자연스럽게 재개됩니다.
